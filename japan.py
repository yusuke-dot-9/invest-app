...
def get_fundamentals(ticker_symbol):
    ...
    try:
        ...
        return {"業種": "取得エラー", "PER": "N/A", "PBR": "N/A", "ROE": "N/A"}
...
with tab1:
    ...
    fundamentals = get_fundamentals(ticker_input) # 業種・財務情報の取得
    ...
    if df is not None:
        ...
        f2.write(f"**PER:** {f'{per:.1f}' if isinstance(per, float) else per} 倍")
        
        pbr = fundamentals['PBR']
        f3.write(f"**PBR:** {f'{pbr:.2f}' if isinstance(pbr, float) else pbr} 倍")
        
        roe = fundamentals['ROE']
        f4.write(f"**ROE:** {f'{roe:.1f}' if isinstance(roe, float) else roe} %")
        
        # LINEボタン
        if st.button("📱 分析結果をLINEに送る"):
            msg = f"【判定】{selected_name}\n結果: {sig}\n株価: {latest['Close']:,.0f}円\n業種: {fundamentals['業種']}\nPER: {fundamentals['PER']}倍\nROE: {fundamentals['ROE']}%"
